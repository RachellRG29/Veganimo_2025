...
function iniciarValidacionCrearReceta() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      if (
        confirm(
          "¿Estás seguro de que deseas salir del formulario? Los datos ingresados se perderán.",
        )
      ) {
        formulario.reset();
  ...
  function validarDescripcion() {
    ...
    if (!texto) {
      ...
      crearAvisoSeccion1(
        descripcion,
        "La descripción debe tener al menos 3 palabras.",
      );
      return false;
    } else {
  ...
  function validarFormulario1() {
    const nombreValido = validarCampoSeccion1(
      nombre,
      "El nombre debe tener al menos 3 letras.",
    );
    const descripcionValida = validarDescripcion();
    const categoriaValida = validarCampoSeccion1(
      categoria,
      "Selecciona una categoría.",
    );
    const tipo_recetaValida = validarCampoSeccion1(
      tipo_receta,
      "Selecciona un tipo.",
    );
  ...
  nombre.addEventListener("input", () => {
    ...
    if (contieneVulgaridad) {
      ...
      crearAvisoSeccion1(
        nombre,
        "Por favor, escribe un nombre válido y respetuoso.",
      );
      return;
    }
  ...
  categoria.addEventListener(
    "change",
    () =>
      intentoEnvio && validarCampoSeccion1(categoria, "Selecciona categoría."),
  );
  tipo_receta.addEventListener(
    "change",
    () =>
      intentoEnvio &&
      validarCampoSeccion1(tipo_receta, "Selecciona tipo receta"),
  );
  estrellas.forEach((e) =>
    e.addEventListener("change", () => intentoEnvio && validarEstrellas()),
  );
  ...
  function validarIngredientes() {
    const inputs = ingredientesZona.querySelectorAll(
    ...
    inputs.forEach((input, i) => {
      ...
      const contieneProhibida = palabrasProhibidas.some((p) =>
        valor.includes(p),
      );
      ...
      if (valor !== "") {
        if (!regexGeneral.test(valor)) {
          ...
          crearAvisoSeccion2(
            input,
            `Ingrediente ${i + 1} contiene palabra no permitida.`,
          );
          todosValidos = false;
        } else if (palabras < 1) {
          aplicarEstiloError(input);
          crearAvisoSeccion2(
            input,
            `Ingrediente ${i + 1} debe tener al menos 1 palabra.`,
          );
          todosValidos = false;
        } else if (palabras > 6) {
          aplicarEstiloError(input);
          crearAvisoSeccion2(
            input,
            `Ingrediente ${i + 1} no puede exceder 6 palabras.`,
          );
          todosValidos = false;
        } else {
          // válido
  ...
  tiempoInput.addEventListener("beforeinput", (e) => {
    ...
    if (
      (tiempoInput.value.length >= 3 && esNumero) ||
  ...
  dificultadSelect.addEventListener(
    "change",
    () => intentoEnvio2 && validarDificultad(),
  );
    ...
    inputs.forEach((input, i) => {
      ...
      if (valor !== "") {
        if (palabras < 2) {
          ...
          crearAvisoSeccion3(
            input,
            `El paso ${i + 1} debe tener al menos 2 palabras.`,
          );
          todosValidos = false;
        } else if (palabras > 50) {
          aplicarEstiloError(input);
          crearAvisoSeccion3(
            input,
            `El paso ${i + 1} excede el máximo de 50 palabras.`,
          );
          todosValidos = false;
          ...
          crearAvisoSeccion3(
            input,
            `El paso ${i + 1} contiene palabras no permitidas.`,
          );
          todosValidos = false;
        } else {
          quitarEstiloError(input);
