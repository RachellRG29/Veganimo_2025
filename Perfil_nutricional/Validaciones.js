document.addEventListener("DOMContentLoaded", () => {
  ...
  function validarSeccionNutricional() {
    let valido = true,
      primerCampoError = null;
    camposNutricionales.forEach(
      ({ element, errorElement, validar, errorMsg, validMsg }) => {
  ...
  function validarDieta() {
    if (checkboxDieta && checkboxDieta.checked) {
      ...
      const contieneMalaPalabra = malasPalabras.some((p) =>
        texto.toLowerCase().includes(p),
      );

      if (texto === "") {
        errorDieta.textContent = "Describe la dieta o desmarca el checkbox.";
      ...
      if (palabras > 250) {
        errorDieta.textContent = "Has excedido 250 palabras.";
        errorDieta.style.display = "block";
        dietaDescripcion.style.borderColor = "red";
        dietaDescripcion.focus();
        return false;
      ...
      if (contieneMalaPalabra) {
        errorDieta.textContent = "Escribe una descripción válida.";
        errorDieta.style.display = "block";
        dietaDescripcion.style.borderColor = "red";
        dietaDescripcion.focus();
        return false;
  ...
  dietaDescripcion.addEventListener("input", () => {
    ...
    if (palabras > 250 || malasPalabras.some((p) => texto.includes(p))) {
      dietaDescripcion.style.borderColor = "red";
    } else {limpiarEstado();}
  });
  ...
  async function validarTarjeta() {
    ...
    const precioPlan = document
      .getElementById("precio_plan")
      .textContent.trim();
    ...
    try {
      ...
      if (!data.success) {
        Swal.fire(
          "Error",
          data.message || "La tarjeta no es válida o el saldo es insuficiente",
          "error",
        );
        return false;
      }
  ...
  async function procesarTodo() {
    ...
    try {
      const response = await fetch(formPerfil.action, {
        method: "POST",
      ...
      Swal.fire({
        icon: "success",
        title: "¡Pago realizado y perfil guardado con éxito!",
        timer: 2000,
  ...
  if (btnEmpezarPlan) {
    ...
    btnEmpezarPlan.addEventListener("click", () => {
      window.location.href =
        "/Pantalla_principal/index_pantalla_principal.html";
    });
  }
});
