...
function manejarBusquedaUsuariosPro(e) {
  ...
  const resultados = listaUsuariosPro.filter(
    (usuario) =>
...
function mostrarUsuariosPro(usuarios) {
  ...
  document.querySelectorAll(".btn-eliminar").forEach((btn) => {
    btn.addEventListener("click", () =>
      confirmarEliminacionPro(btn.dataset.id),
    );
  });
}
...
function filtrarUsuariosPro() {
  ...
  const usuariosFiltrados = listaUsuariosPro.filter(
    (user) =>
      user.email.toLowerCase().includes(filtro) ||
      user.nombre_completo.toLowerCase().includes(filtro),
  );
      ...
      .then((data) => {
        if (data.success) {
          ...
          Swal.fire({
            icon: "success",
            title: nuevoEstado
              ? "Usuario PRO bloqueado"
              : "Usuario PRO desbloqueado",
            toast: true,
            position: "bottom-end",
            showConfirmButton: false,
            timer: 3000,
          ...
          Swal.fire({
            ...
            title: "Error",
            text:
              data.error || "No se pudo actualizar el estado del usuario PRO",
          });
        }
      })
      .catch((err) =>
        Swal.fire({
          ...
          title: "Error",
          text:
            err.message || "No se pudo actualizar el estado del usuario PRO",
        }),
      );
  });
}
...
function confirmarEliminacionPro(idUsuario) {
  const usuario = listaUsuariosPro.find(
    (u) => (u.usuario_id || u._id) === idUsuario,
  );
  if (!usuario) {return;}
...
function eliminarUsuarioPro(idPerfil) {
  fetch(`/Portal_de_administrador/usuariospro.php?id=${idPerfil}`, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
