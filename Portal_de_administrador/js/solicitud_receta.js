document.addEventListener("DOMContentLoaded", async function () {
  console.log(
    "📦 solicitud_receta.js cargado - Esperando contenido dinámico...",
  );
...
function inicializarSolicitudes() {
  ...
  if (inputBusqueda) {
    inputBusqueda.addEventListener("input", function () {
      ...
      const filtradas = solicitudesData.filter(
        (sol) =>
          (sol.nombre_receta &&
            sol.nombre_receta.toLowerCase().includes(texto)) ||
  ...
  function renderizarTabla(lista) {
    ...
    document
      ...
      .forEach((btn) =>
        btn.addEventListener("click", () => verSolicitud(btn.dataset.id)),
      );
    document
      .querySelectorAll(".btn-aprobar")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          actualizarEstado(btn.dataset.id, "Aprobada"),
        ),
      );
    document
